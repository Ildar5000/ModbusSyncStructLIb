        ...
        public void Open()
        {
            try
            {
                if (TypeModbus == 0)
                {
                    ...
                    slave = ModbusSerialSlave.CreateRtu(slaveID, serialPort,1);
                ...
                if (TypeModbus == 1)
                {
                    ...
                    slave = ModbusSerialSlave.CreateAscii(slaveID, serialPort, 1);
                    slave.Transport.WriteTimeout = 1000;
                    slave.Transport.ReadTimeout = 1000;
                ...
                if (TypeModbus==2)
                {
                    ...
                    slave = ModbusTcpSlave.CreateTcp(slaveID, ListenerTCP, 1, 5,1,"1",false);
                    logger.Info("Slave подключен");
